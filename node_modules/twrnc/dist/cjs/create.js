...
const tw_config_1 = require("./tw-config");
...
function create(customConfig, platform, reactNativeVersion) {
    ...
    function color(utils) {
        ...
        if (config.theme) {
            ...
            for (const key of Object.keys(tw_config_1.PREFIX_COLOR_PROP_MAP)) {
                const prefix = key;
                const suffix = utils.slice(prefix.length);
                const themePropertyName = tw_config_1.PREFIX_COLOR_PROP_MAP[prefix];
                const themeColors = config.theme[themePropertyName];
                if (suffix && utils.startsWith(prefix) && themeColors) {
                    color = (0, color_1.configColor)(suffix, themeColors);
                    if (color) {
                        return color;
                    }
                }
            ...
            if (!/\s+/.test(utils) && !utils.includes(`/`) && config.theme.colors) {
                color = (0, color_1.configColor)(utils, config.theme.colors);
                if (color) {
                    return color;
                }
            }
